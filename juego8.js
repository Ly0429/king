...
//sonido pasos
const sonidoPaso = new THREE.Audio(listener);

audioLoader.load("step.mp3", function (buffer) {
    sonidoPaso.setBuffer(buffer);
    sonidoPaso.setLoop(true);
...
function crearEnemigo() {
    ...
    enemigo.position.y = Math.random() * 5 + 3;
...
if (jugador.position.x > 135) {
    ...
    if (moviendose && enSuelo) {
        if (sonidoPaso.buffer && !sonidoPaso.isPlaying) {
            sonidoPaso.play();
        }
    } else {
        if (sonidoPaso.isPlaying) {
            sonidoPaso.stop();
        }
    }
